...
- 提供一个通用的 `deliver_notification(notification_id)` 业务函数（在 `app/services/delivery.py`）；
- `app/tasks/delivery.py` 提供 `register_provider_actor(queue_name)`：把通用业务函数包装成一个 Dramatiq actor，actor 名以 `deliver_<queue_name>` 命名，actor 的 `queue_name` 取参数；多次注册同一个 queue 是 idempotent 的；
- worker 启动入口（`scripts/run_worker.py`）启动时同步读取 `providers` 表中所有 `enabled=true` 的供应商，去重得到 queue 列表，逐个调用 `register_provider_actor`，然后把 queue 列表透传给 Dramatiq CLI 的 `--queues` 参数；
- API 进程入队时调 `actor_for_queue(queue_name).send(notification_id)`；本进程第一次调用某个 queue 时 lazy 注册，后续复用。

第一版**不要**做运行时动态新增队列：

- **不要**在 API 进程接到 admin 请求时去动态创建队列、向 worker 广播队列变化；
- **不要**让 worker 在运行中轮询 DB 检测新增供应商；
- 新增供应商后，必须重启 worker 才会监听新队列；
- 如果 worker 启动时 `providers` 表内没有任何 `enabled=true` 记录，启动脚本应直接退出并报错，不要静默启动一个不监听任何队列的空 worker。
...
1. **httpx 请求超时**：`HTTP_REQUEST_TIMEOUT_SECONDS`（默认 10 秒），覆盖 connect、read、write 全过程，超时时 httpx 抛超时异常，被 worker 捕获、按失败记录 `delivery_attempts`（错误类型 `timeout`），然后按正常重试流程进入下一次尝试。
2. **Dramatiq actor 总执行时长上限**：`ACTOR_TIME_LIMIT_SECONDS`（默认 30 秒），作为兜底防止 worker 卡在 DB IO、adapter 逻辑或其他非 httpx 路径上。**不要**把它配得比 `HTTP_REQUEST_TIMEOUT_SECONDS` 小，留出至少 3–5 倍的余量。

第一版接受 `time_limit` 命中时可能留下 `delivering` 状态的任务（actor 被强杀，应用层没机会改状态）。**不要**为此额外实现 stale delivering recovery 机制；当作罕见运维事件，必要时人工 SQL 修复。
...
- 首次入队：返回 `202 Accepted`，`message=accepted`，body 包含 `notification_id`、当前状态（`pending`）和 `idempotency.deduplicated=false`；
- 完全重复提交：相同 `provider_code + event_type + event_id` 命中唯一约束、且 `payload` / `metadata` 与已有记录完全相同时，返回 `200 OK`，`message=duplicate_accepted`，`idempotency.deduplicated=true`，**不会重新入队**。用 `200` 而不是 `202` 是为了让调用方能明确区分"刚创建"和"命中已有"；
- 幂等冲突：相同 `provider_code + event_type + event_id` 命中唯一约束、但 `payload` 或 `metadata` 与已有记录不一致时，返回 `409 idempotency_conflict`，`data` 字段包含 `existing_notification_id`，避免业务方误以为新内容已被处理；
...
- Worker 启动入口（`scripts/run_worker.py`）从 `providers` 表读取 enabled 的 queue 列表，调用 `register_provider_actor` 注册对应 actor 后再启动 Dramatiq CLI。**不要**把 queue 列表硬编码进代码或 docker-compose `command`；
- Worker actor `time_limit` 作为 httpx 超时之外的兜底，命中时接受可能留下 `delivering` 卡死任务，**不要**为此实现 stale recovery 机制；
- 错误响应统一走 `app/errors.py` 的 handler：自定义业务错误（`AppError`）、Pydantic 校验失败、FastAPI / Starlette 内部 `HTTPException`、未捕获的 `Exception` 都返回 `{code, message, data}` 结构，**不要**让任何路径绕过这个 handler 直接返回 `{"detail": ...}` 原生格式。
